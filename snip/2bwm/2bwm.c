            ...
            if (NULL != client) {
                ...
                if (ws == NET_WM_FIXED) {
                    ...
                    if (TWOBWM_NOWS != ws && ws < WORKSPACES) {
                        ...
                        if (ws != curws)
                            xcb_unmap_window(conn, client->id); /* If it's not our current workspace, hide it. */
                    } else {
...
bool setup(int scrno)
{
    ...
    xcb_create_window(conn, XCB_COPY_FROM_PARENT, cwin, screen->root, 0, 0, screen->width_in_pixels, screen->height_in_pixels
