...
struct apple_push
{
    ...
    struct push_data
    {
        ...
        void pop_data() { data_.clear(); }
        void append(const std::string& m) { data_ += m; }
    ...
    typedef std::map<connection*,push_data_ptr> bufs_type;
    bufs_type bufs_;
    ...
    {
        bufs_type::iterator i = insert_message(bufs_.end(), m, &c);
        pmessage(i, m);
        return boost::bind(&apple_push::pmessage, this, i, _1);
    }

private:
    bufs_type::iterator insert_message(bufs_type::iterator i, connection::message const & m, connection *c)
    {
        if (m.error())
        {
            if (i != bufs_.end())
        ...
        {
            BOOST_ASSERT(c);
            std::pair<bufs_type::iterator,bool> p = bufs_.insert(std::make_pair(c, push_data_ptr()));
            i = p.first;
            if (p.second)
            {
                i->second.reset(new push_data(c));
            }
        }
        i->second->append(m.encode());
        return i;
    }

    void pmessage(bufs_type::iterator i, connection::message const & m)
    {
        insert_message(i, m, 0);
    ...
    void write_msg()
    {
        ...
        pcur_ = bufs_.begin()->second;
        stage_ = stage::writing;
        boost::asio::async_write(socket_
                , boost::asio::buffer(pcur_->data())
    ...
    void handle_write(sys::error_code const & ec)
    {
        ...
        if (pcur_->connected())
        {
            ...
            pcur_->get_connection().send(boost::asio::buffer(replybuf_)
    ...
    {
        stage_ = stage::ready;
        pcur_->pop_data(); // bufs_.pop_front();
        write_msg();
    }
