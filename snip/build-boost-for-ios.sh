### https://svn.boost.org/trac/boost/ticket/10742
...
: ${BOOST_LIBS:="coroutine random regex graph random chrono thread signals filesystem system date_time"}
...
downloadBoost()
{
    if [ ! -s $TARBALLDIR/boost_${BOOST_VERSION2}.tar.bz2 ]; then
        ...
        curl -L -o $TARBALLDIR/boost_${BOOST_VERSION2}.tar.bz2 sourceforge.net/projects/boost/files/boost/${BOOST_VERSION}/boost_${BOOST_VERSION2}.tar.bz2/download
    fi
