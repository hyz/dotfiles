...
struct Pump
{
    ...
    static void dispatch(this_type* thiz)
    {
        ...
        value_t val; //boost::asio::streambuf buf
        std::cout <<"\ndispatch " << X <<" "<< typeid(value_t).name();

        typedef typename detail::gens<std::tuple_size<typename std::remove_reference<value_t>::type>::value>::type Seqs;

        Seqs::invoke(std::forward<Handle>(thiz->handle_), std::forward<value_t>(val));
    }

    void dispa_byid(int i) { disp_fns_[i](this); }

    template <typename... T> using tuple_no_cv = std::tuple<typename std::remove_reference<T>::type...>;

    template <typename... Tlist>
    void send(Tlist&&... l)
    {
        std::cout <<"\nsend " << tuple_index<Tab, tuple_no_cv<Tlist...>>::value;
    }
...
using MessageIndex = tuple_index<Tab,std::tuple<typename std::remove_reference<Args>::type...>>;
...
struct MessageHandle
{
    ...
    void operator()(Args&&...) {
        std::cout <<"\nnot handled " << MessageIndex<Tab,Args...>::value;
    }
    ...
    void operator()(Id, int, std::string&) {
        std::cout <<"\nId int string";
    }
};
...
int main()
{
    ...
    Pump<MessageTable, MessageHandle<MessageTable>> pump;

    pump.send(char(0));
    pump.send(int(0));
    pump.send(float(0));
    pump.send(double(0));
    pump.send(int(0), string());
    pump.send(Id{}, int(0), string());

    for (int i=0; i < tuple_size<MessageTable>::value; ++i) {
        pump.dispa_byid(i);
    }
