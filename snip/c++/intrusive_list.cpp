...
int main()
{
   ...
   for(int i = 0; i < 100; ++i) {
       ...
       BOOST_ASSERT( MemberList::node_algorithms::inited( &values.back().member_hook_ ) );
   }
   ...
   for(VectIt it(values.begin()), itend(values.end()); it != itend; ++it)
   {
       // MemberList::node_ptr nptr = memberlist.get_real_value_traits().to_node_ptr(*it);
       BOOST_ASSERT( MemberList::node_algorithms::inited( &it->member_hook_ ) );
