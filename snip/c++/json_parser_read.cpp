...
namespace boost { namespace property_tree { namespace json_parser
{
    ...
    struct context
    {
        ...
        struct a_name
        {
            ...
            {
                std::cout << std::string(b+1,e-1) << " --- a_name\n";
                // (c.stack().back().first);
        ...
        struct a_string_val
        {
            ...
            void operator()(It b, It e) const
            {
                std::cout << std::string(b+1, e-1) << " --- a_string_val\n";
                            ...
                            const std::string &filename)
    {
        ...
        if (!stream.good())
            throw 1; //BOOST_PROPERTY_TREE_THROW(json_parser_error("read error", filename, 0));
        ...
        {
            throw; // BOOST_PROPERTY_TREE_THROW(json_parser_error(e.descriptor, filename, count_lines<It, Ch>(v.begin(), e.where)));
        }
...
int main(int argc, char *const argv[])
{
    ...
    boost::property_tree::json_parser::read_json_internal(std::cin, pt, "");
    // std::cout << pt; //.get<std::string>(argv[1]) << "\n";
    return 0;
}
