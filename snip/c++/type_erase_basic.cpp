...
void xxmain()
{
    ...
    std::cout << boost::make_iterator_range(lis) <<"\n";
}


BOOST_TYPE_ERASURE_MEMBER((has_foo), foo, 0)
BOOST_TYPE_ERASURE_MEMBER((has_bar), bar, 1)
BOOST_TYPE_ERASURE_MEMBER((has_hello), hello, 2)
BOOST_TYPE_ERASURE_MEMBER((has_get), get, 0)
BOOST_TYPE_ERASURE_MEMBER((has_info), info, 0)

typedef boost::type_erasure::any< mpl::vector<
            has_foo<void()>
          , has_bar<int(int)>
          , has_hello<void(int,std::string const&)>
          , has_get<int()>
          , has_info<std::string()>
        > , _self&> Any;

void hello(Any const& a)
{
    std::cout << sizeof(a) <<"\n";
    a.foo();
    std::cout << a.bar(1);
...
struct A : boost::noncopyable
{
    ...
    void hello(int x,std::string const&) { std::cout<<"HELLO"<<x;}
