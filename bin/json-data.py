...
def country_code():
    # https://raw.githubusercontent.com/mohuilin/CountryCode/master/chineseCountryJson.txt
    def fmt(phoneCode,countryName,countryCode, **kwa):
        print(f"INSERT INTO res_country(code,name,phone_code) values('{countryCode}','{countryName}',{phoneCode});")
    js = json.load(sys.stdin)
    itemlis = js['data']
    for item in itemlis:
        try:
            item["phoneCode"] = int( item["phoneCode"] )
            fmt(**item)
        except:
