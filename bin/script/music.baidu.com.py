...
def modificate_file_name_for_wget(file_name):
    file_name = re.sub(r'\s*:\s*', ' - ', file_name)    # for FAT file system
...
class baidu_music(object):
    def __init__(self, url):
        ...
        self.template_wgets = 'wget -nv -U "%s" -O \'%%s.tmp\' \'%%s\'' % (headers['User-Agent'])
    ...
    def get_songidlist(self, id_):
        ...
        api_json = json.loads(api_json.decode('utf8'))
    ...
    def modified_id3(self, file_name, info):
        ...
        id3.add(COMM(encoding=3, desc='Comment', text=info['song_url']))
        if info['album_pic_url']:
            id3.add(APIC(encoding=3, mime='image/jpg', type=3, desc='Cover', data=self.get_cover(info['album_pic_url'])))
        id3.save(file_name)
    ...
    def url_parser(self):
        if '/album/' in self.url:
            ...
            # print((s % (2, 91, '   请正确输入baidu网址.')))
            fp = urllib.parse.urlparse(self.url).path.strip('/\\')
            if not os.path.exists(fp + '.tmp'):
                os.makedirs(os.path.dirname(fp), exist_ok=True)
                wget = self.template_wgets % (fp, self.url)
                print(wget)
                ...
                if status != 0:     # other http-errors, such as 302.
                    ...
                    print(('  ===> ' + wget))
                    sys.exit(4)
            bs = BeautifulSoup( open(fp + '.tmp').read() )
            for i in bs.find_all('li'): #(, {'data-songitem':has_songitem})
                try:
                    ...
                    self.template_api = 'http://music.baidu.com/data/music/fmlink?songIds=%s&type=mp3&rate=320'
                    # self.template_api = 'http://music.baidu.com/data/music/fmlink?songIds=%s&type=mp3'
                    if i.find('a', {'class':'icon-lossless'}):
                        si['format'] = 'flac'
                        self.template_api = 'http://music.baidu.com/data/music/fmlink?songIds=%s&type=flac'
                    file_name = '{sname} - {author}.{format}'.format(**si)
    ...
    def get_song_infos(self, file_ext='.mp3'):
        print(self.template_api % self.song_id)
        api_json = self.opener.open(self.template_api % self.song_id).read()
        j = json.loads(api_json.decode('utf8'))
        print(j)
        song_info = {}
        song_info['song_id'] = str(j['data']['songList'][0]['songId'])
        song_info['track'] = ''
        song_info['song_url'] = 'http://music.baidu.com/song/' + song_info['song_id']
    ...
    def get_album_infos(self):
        ...
        for i in songidlist:
            song_info = {}
            song_info['song_id'] = str(i['songId'])
            song_info['song_url'] = 'http://music.baidu.com/song/' + song_info['song_id']
            song_info['track'] = str(ii)
        ...
        self.dir_ = os.path.join(os.getcwd(), d)
        self.download()
    ...
    def download(self):
        ...
        for i in self.song_infos:
            ...
            if 'zhangmenshiting.baidu.com' in i['durl'] \
                    or 'yinyueshiting.baidu.com' in i['durl'] \
                    or 'qianqian.com' in i['durl']:
