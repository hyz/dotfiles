...
entries.each do |e|
  ...
  rec = Record.new(name, url, username, password, extra, grouping, fav)
  #records << rec
  passnames[name] += [rec]
end

#puts "Records parsed: #{records.length}"
puts "Records parsed: #{passnames.length}"

successful = 0
errors = []
passnames.each_pair do |k,v|
  print "Creating record #{k}..."
  #puts "pass insert -m#{"f" if FORCE} '#{k}'"
  #puts v.join("\n")
  IO.popen("pass insert -m#{"f" if FORCE} '#{k}' > /dev/null", 'w') do |io|
    io.puts v.join("\n")
  end
  if $? == 0
    puts " done!"
