...
struct rtph264_client : boost::noncopyable
{
    ...
    rtph264_client(boost::asio::io_service& io_s
            , FILE* outfile
            , int port, std::string sps, std::string pps)
...
private:
    ...
    std::unique_ptr<FILE,decltype(&fclose)> fp_;//(fp, fclose);
...
struct Main : boost::asio::io_service
{
    ...
    {
        Args args(ac, av);
        new (&objmem_) Object(*this, args.fp, args.port, args.sps, args.pps);
    }
...
private:
    static int objmem_[sizeof(Object)/sizeof(int)+1];
    static Object* obj() { return reinterpret_cast<Object*>(&objmem_); }
};
int Main::objmem_[sizeof(Object)/sizeof(int)+1] = {};
