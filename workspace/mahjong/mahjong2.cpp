...
//   if (size >= 1) {
//       SDL_RenderCopy(renderer, m_imgTiles, &dstrect2, &dstrect);
...
View_resource::View_resource()
{
    ensure(SDL_Init(SDL_INIT_VIDEO|SDL_INIT_AUDIO|SDL_INIT_TIMER|SDL_INIT_NOPARACHUTE) >=0, "SDL_Init: %s", SDL_GetError());
    ...
    //ensure(window = SDL_CreateWindow("Mahjong", SDL_WINDOWPOS_UNDEFINED,0, wh,wh, SDL_WINDOW_RESIZABLE), "SDL_CreateWindow: %s", SDL_GetError()); //SDL_WINDOW_BORDERLESS,SDL_WINDOW_FULLSCREEN_DESKTOP,SDL_WINDOW_FULLSCREEN
    ensure(window = SDL_CreateWindow("Mahjong", SDL_WINDOWPOS_UNDEFINED,0, wh,wh, 0), "SDL_CreateWindow: %s", SDL_GetError()); //SDL_WINDOW_BORDERLESS,SDL_WINDOW_FULLSCREEN_DESKTOP,SDL_WINDOW_FULLSCREEN
    ensure(renderer = SDL_CreateRenderer(window, -1, SDL_RENDERER_PRESENTVSYNC), "SDL_CreateRenderer: %s", SDL_GetError());
    ...
    : res(&vr)
{
    ...
    texture_ = SDL_CreateTexture(res->renderer, SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_TARGET, wh_, wh_);
    ...
    : Renderer(vr, 4*vr.tiles.size())
{
    ...
    //SDL_GetRendererOutputSize(renderer, &w, &h);SDL_GetWindowSize(window, &w, &h);SDL_RendererGetViewport(renderer, &rect);
    //squa_.w = squa_.h = std::min(dm.w,dm.h);
    //squa_.x = squa_.y = 0;

    //SDL_TEXTUREACCESS_STREAMING SDL_TEXTUREACCESS_STATIC
    //texture_ = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_TARGET, squa_.w, squa_.h);
    //SDL_SetTextureBlendMode(texture_, SDL_BLENDMODE_BLEND);

    //for (int i=0; i<4; ++i)
    //    facing_[i] = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_TARGET, squa_.w, squa_.h/2);
}
...
void View::draw_wall(Mahjong::Tiles const& tiles, int pos0)
{
    ...
    int w = w_+1;
    int y = wh_/2 - (nline_-2)*w;
    int x = wh_/2 - (nline_-6)*w;

    SDL_Rect dr = {x,y, w_,h_};
    int nt = tiles.size();
    int nr = nt/4/2;
    ...
    for (int pos=pos0; pos < pos0+4; ++pos) {
        ...
        for (int i=beg; i < end; ++i) {
            if (tiles.first_ <= i && i <= last) {
                ...
                SDL_RenderCopy(res->renderer, tex, 0, &rect);
            }
        }
        SDL_SetRenderTarget(res->renderer, 0);
        //SDL_SetRenderDrawColor(res->renderer, 0,0,0, 255);
        ...
        SDL_RenderCopyEx(res->renderer, texture_, 0, &rect, (pos % 4)*90+180, 0, SDL_FLIP_NONE);
        //break;
    }
    SDL_SetRenderTarget(res->renderer, 0);
}
