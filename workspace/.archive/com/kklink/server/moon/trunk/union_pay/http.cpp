...
template <typename I>
inline std::string unescape(I b, I end)
{
    return urldecode(std::string(b, end));
}

template <class C>
struct basic_keyvalues : C
{
    typename C::iterator find(const std::string& k) const
    {
        return const_cast<basic_keyvalues*>(this)->find(k);
    }
    typename C::iterator find(const std::string& k)
    {
        typename C::iterator i = this->begin();
        for (; i != this->end(); ++i)
    ...
    typename C::iterator insert(typename C::value_type const & kv)
    {
        return C::insert(this->end(), kv);
    }

    template <typename I> basic_keyvalues(I b, I e) : C(b, e) {}
    basic_keyvalues() {}
};

typedef basic_keyvalues<std::list<std::pair<std::string, std::string> > > keyvalues;

template <typename C, typename I>
basic_keyvalues<C> & parse_keyvalues(basic_keyvalues<C> &params, I beg, I end)
{
    I i;
    I eop;
    while (beg != end)
    {
        ...
        if (i == eop)
            break;
        params.insert(std::make_pair(unescape(beg, i), unescape(i+1, eop)));
        if (eop == end)
            break;
...
inline basic_keyvalues<C> & parse_keyvalues(basic_keyvalues<C> &params, V const & v)
{
    return parse_keyvalues(params, v.begin(), v.end());
}
...
template <typename T>
inline void put(basic_keyvalues<T> &l, const std::string& name, std::string const & val)
{
    l.insert(std::make_pair(name, val));
}
template <typename T>
inline void put(basic_keyvalues<T> &l, const std::string& name, char const * val)
{
    l.insert(std::make_pair(name, std::string(val)));
}
template <typename T,typename V>
inline void put(basic_keyvalues<T> &l, const std::string& name, V const & x)
{
    l.insert(std::make_pair(name, boost::lexical_cast<std::string>(x)));
}

template <typename T>
inline void erase(basic_keyvalues<T> &l, const std::string& name)
{
    typename basic_keyvalues<T>::iterator i = l.find(name);
    if (i != l.end())
        l.erase(i);
}
...
{
    std::string::const_iterator i, b, end;
    ...
    if (b != end)
        parse_keyvalues(params, b+1, end);
    // while (b != end)
    // {
    //     ++b;
    //     eop = std::find(b, end, '&');
...
struct http_server : acceptor<http_server>
{
    ...
    typedef boost::function<sys::error_code (connection&, sys::error_code const &, message const &)> messager_fn0;
...
private:
    ...
    void handle_error(iterator i, sys::error_code ec, int ln,char const *fn);
...
//sys::error_code http_server::_request(iterator i, message const & m)
//{
//    sys::error_code ec = messager0_(*i->second, sys::error_code(), m);
//    if (ec)
//    {
...
//            LOG_I << ep;
//        sh->socket().close(ec);
//        handle_error(i, ec, __LINE__,__FILE__);
//        cs_.erase(i);
//    }
//    return ec;
//}

void http_server::handle_error(iterator i, sys::error_code ec, int ln,char const *fn)
{
    LOG_I << ec <<" "<< ec.message();
    if (ec == boost::asio::error::operation_aborted
    ...
    {
        ec = make_error_code(boost::asio::error::message_size);
    }
    ...
    messager0_(*i->second, ec, message());
...
void http_server::handle_content(iterator i, sys::error_code const & ec)
{
    ...
    sys::error_code erc = messager0_(*i->second, sys::error_code(), sh->message_);
    if (erc)
    {
        // tcp::endpoint ep = sh->socket().remote_endpoint(erc);
        sh->socket().close(erc);
        cs_.erase(i);
        return;
    }
...
void http_server::handle_method_line(iterator i, sys::error_code const &ec)
{
    ...
    {
        if (ec == boost::asio::error::eof)
            if (!messager0_(*i->second, ec, message()))
                return;
...
{
    if (iself_ == server_->cs_.end())
        return;

    LOG_I << static_cast<void*>(this) <<" "<<ln <<":"<<fn <<" "<< __FILE__;

    server_->get_io_service().post(boost::bind(&http_server::ierase, server_, iself_));
    iself_ = server_->cs_.end();
