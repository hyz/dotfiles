...
static unsigned short port_ = 9991;
...
static std::string getpwd(std::size_t max_length, boost::asio::ssl::context::password_purpose purpose) { return "abc123"; }
...
private:
    ...
    void timed_connect(sys::error_code ec)
    {
        ...
        if (stage_ != stage::connecting)
            return;
        ssl_.reset(new wrapssl(timer_.get_io_service()));
...
private:
    ...
    {
        explicit wrapssl(boost::asio::io_service & io_service)
            : context_(boost::asio::ssl::context::sslv3)
    ...
    private:
        ...
        static boost::asio::ssl::context & init_context(boost::asio::ssl::context & ctx)
        {
...
int main(int ac, char* av[])
{
    ...
    try
    {
        ...
        s.start(tcp::endpoint(tcp::v4(), port_), boost::bind(&apple_push::message, &ap, _1, _2));
