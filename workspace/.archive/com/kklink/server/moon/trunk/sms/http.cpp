...
static boost::system::error_category & _errcat()
{
    return myerror_category::inst<myerror_category>();
}
...
std::string get(keyvalues const &l, const std::string& name)
{
    keyvalues::const_iterator i = l.find(name);
    if (i == l.end())
...
std::string response::encode_header() const
{
    ...
    for (keyvalues::const_iterator it = headers_.begin(); it != headers_.end(); ++it)
    {
...
sys::error_code request::parse_header_lines(std::streambuf & sbuf)
{
    ...
    while (getline(ins, line) && !line.empty() && line[0] != '\r')
    {
        ...
        {
            return sys::error_code(http::error::invalid_header_line,_errcat());
        }
...
sys::error_code request::parse_request_line(std::streambuf & sbuf)
{
    ...
    {
        return sys::error_code(http::error::invalid_path_params,_errcat());
    }
    LOG_I << params_;
...
struct http_connection : boost::noncopyable
{
    ...
    typedef boost::function<void (message const&)> message_handler;
...
struct http_server : acceptor<http_server>
{
    ...
    typedef boost::function<void (message const &)> messager_fn;
...
private:
    ...
    void _request(iterator i, message const & m);
...
void http_server::_request(iterator i, message const & m)
{
    if (i->second->messager_)
    {
        i->second->messager_(m);
        return;
    }
    i->second->messager_ = messager0_(m, *i->second);
}
...
void http_server::handle_content(iterator i, sys::error_code const & ec)
{
    ...
    sh->message_.content(sh->rsbuf_);
    _request(i, sh->message_);
...
void http_server::handle_headers(iterator i, sys::error_code const & ec)
{
    ...
    if (sh->message_.content().size() <= sh->rsbuf_.size())
    {
        return handle_content(i, ec);
    }

    boost::asio::async_read(sh->socket(), sh->rsbuf_
            , boost::asio::transfer_at_least(sh->message_.content().size() - sh->rsbuf_.size())
