...
void logger_stream<Conf>::commit(int line, char const *name)
{
    ...
    if (bufs.size() > sizepfx)
    {
        if (syslog_)
        {
            ...
            ::syslog(LOG_USER|Conf::level, fmt, len, p, line, name);
            // if (Conf::level == LOG_ERR) { ; }
        }
        else
        {
...
struct Error
{
    BOOST_STATIC_CONSTANT(int, level=LOG_ERR);
    BOOST_STATIC_CONSTANT(int, capacity=1024);
};
...
#undef LOG_ERR
#define LOG_ERR if(1)logging::logger_helper<logging::Error>(__LINE__,__FUNCTION__)
...
struct Milli_ {
    BOOST_STATIC_CONSTANT(int, multi=1000);
    BOOST_STATIC_CONSTANT(int, subti=1000000);
};
struct Micro_ {
    BOOST_STATIC_CONSTANT(int, multi=1000000);
    BOOST_STATIC_CONSTANT(int, subti=1000);
};
template <typename M>
uint64_t Seconds(M const& res)
{
    struct timespec ts;
    clock_gettime(CLOCK_REALTIME, &ts);
    return ts.tv_sec * uint64_t(res.multi) + ts.tv_nsec / uint64_t(res.subti);
}

inline time_t   Seconds() { return time(0); }
inline uint64_t Microseconds() { return Seconds(Milli_()); }
inline uint64_t Milliseconds() { return Seconds(Micro_()); }
