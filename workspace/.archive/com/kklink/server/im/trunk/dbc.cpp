...
int exec(boost::system::error_code & ec, const std::string& sql, const dbc::connection& wc)
{
    ...
    if (mysql_query(c, sql.c_str()) != 0)
    {
        ...
        ec = make_error_code(boost::asio::error::fault); // BOOST_THROW_EXCEPTION( sql::error_query() );
        return -1;
    }
...
redis::reply context_helper::reply()
{
    ...
    if (ret != REDIS_OK)
    {
        ...
        n_reply_ = 0;
        context_ptr_.reset();
        LOG_ERR << "redis redisGetReply !REDIS_OK";
        return redis::reply();
    }
...
void Sql_Exec(boost::format const& s)
{
    // sql::exec(s);
    redis::command("LPUSH", "myq", str(s));
}

void Sql_Exec(std::string const& s)
{
    // sql::exec(s);
    redis::command("LPUSH", "myq", s);
}
