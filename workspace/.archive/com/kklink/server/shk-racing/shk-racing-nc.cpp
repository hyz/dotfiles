#include <cstdlib>
#include <cstring>
#include <vector>
#include <iostream>
#include <boost/asio.hpp>
#include <boost/algorithm/string/trim.hpp>

using boost::asio::ip::tcp;

static volatile int end_ = 0;

static void on_signal(const boost::system::error_code& er, int sig)
{
    std::cout << er <<" "<< sig <<"\n";
    exit(0);
}

int main(int argc, char* argv[])
{
    try
    {
        if (argc != 4) {
            std::cerr << "Usage: yx-shk-nc <host> <port>\n";
            return 1;
        }

        boost::asio::io_service io_service;

        boost::asio::signal_set signals(io_service, SIGHUP, SIGUSR1, SIGUSR2);
        signals.async_wait(&on_signal);

        tcp::socket sk(io_service);
        union { uint32_t len; char s[sizeof(uint32_t)]; } un;
        char opx = (char)atoi(argv[3]);

        {
            tcp::resolver resolver(io_service);
            tcp::resolver::query query(tcp::v4(), argv[1], argv[2]);
            tcp::resolver::iterator iterator = resolver.resolve(query);

            boost::asio::connect(sk, iterator);
        } {
            std::string request(std::istream_iterator<char>(std::cin) , std::istream_iterator<char>());
            boost::algorithm::trim(request);

            un.len = htonl(request.size());
            boost::asio::write(sk, boost::asio::buffer(&opx, 1));
            boost::asio::write(sk, boost::asio::buffer(un.s));
            boost::asio::write(sk, boost::asio::buffer(request));
        }
        while ( 1 ) {
            std::vector<char> response;

            boost::asio::read(sk, boost::asio::buffer(&opx, 1));
            boost::asio::read(sk, boost::asio::buffer(un.s));
            response.resize(un.len = ntohl(un.len));
            boost::asio::read(sk, boost::asio::buffer(response));

            std::cout << int(opx) <<" "<< un.len <<": ";
            std::cout.write(&response[0], response.size());
            std::cout.flush();
        }
    }
    catch (std::exception& e)
    {
        std::cerr << "Exception: " << e.what() << "\n";
        return 2;
    }

    return 0;
}

