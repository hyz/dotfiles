...
static volatile int stat_ = 0;

enum Payload_Type {
    PT_DepthMarketData = 11
};

template <typename T, int Np> struct AlignBuf {
    int h;
    T obj;
    char pad[sizeof(int) - Np];
};
template <typename T> struct AlignBuf<T,0> {
    int h;
    T obj;
};
template <typename T>
void Write(int t, T* obj)
{
    AlignBuf<T, sizeof(T)%sizeof(int)> buf;
    buf.h = (t << 24) | (sizeof(buf) - sizeof(int));
    memcpy(&buf.obj, obj, sizeof(T));
    int n = write(fifo_, &buf, sizeof(buf));
    if (n != sizeof(buf)) {
        //fifo_reopen();
        ERR_EXIT("write %lu: %d %s", sizeof(buf), n, strerror(errno));
    }
}
...
void CtpMdSpi::SubscribeMarketData(char const* ids[], int len)
{
//    vector<char*> list;
...
//        pInstId[i]=list[i];
//
    int ret=pUserApi->SubscribeMarketData((char**)ids, len);
...
void CtpMdSpi::OnRtnDepthMarketData(CThostFtdcDepthMarketDataField *pDepthMarketData)
{
    ...
    Write(PT_DepthMarketData, pDepthMarketData);
...
void test_md(void)
{
    ...
    char const* idlist[] = { "ag1606" };
    //strcpy(idlist, "ag1606");
    //cerr<<" 合约 > "; cin>>idlist;
    pUserSpi->SubscribeMarketData(idlist, sizeof(idlist)/sizeof(idlist[0]));
...
{
    int helo = 0;
    if (write(fd, &helo, sizeof(int)) != sizeof(int))
        ERR_EXIT("hello");
}
static void stat_wait(int st, int sec=4)
{
    int ms = sec * 10;
    while (st == stat_ && ms > 0) {
        usleep(1000*100); //usleep//nanosleep;
        sec--;
    }
    if (st == stat_) {
        ERR_EXIT("wait %d sec %d timeout", st, sec);
    }
}

static void fifo_reopen()
{
    if (fifo_ >= 0)
        close(fifo_);

    remove(fifo_path_);
    mkfifo(fifo_path_, 0600); // std::unique_ptr<FILE,decltype(&remove)> xclose(fp, remove);

    fifo_ = open(fifo_path_, O_WRONLY);
    if (fifo_ < 0) {
        ERR_EXIT("open %s: %s", fifo_path_, strerror(errno));
    }
    hello(fifo_);
}
...
int main(int argc, char* const argv[])
{
    ...
    CThostFtdcMdApi* pUserApi = CThostFtdcMdApi::CreateFtdcMdApi();
    {
        ...
        stat_ = stat_login;
        pUserSpi->ReqUserLogin(appId,userId,passwd);
        stat_wait(stat_login);

        //char idlist[100];
        //strcpy(idlist, "ag1606");
        //cerr<<" 合约 > "; cin>>idlist;
        stat_ = stat_subscribe;
        char const* idlist[] = { "ag1606" };
        pUserSpi->SubscribeMarketData(idlist, sizeof(idlist)/sizeof(idlist[0]));
        stat_wait(stat_subscribe);
    }
