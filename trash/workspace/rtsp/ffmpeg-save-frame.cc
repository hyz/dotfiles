...
int main(int argc, char **argv)
{
    ...
    img_convert_ctx = sws_getCachedContext(img_convert_ctx, pCodecCtx->width, pCodecCtx->height,
            pCodecCtx->pix_fmt, pCodecCtx->width, pCodecCtx->height, AV_PIX_FMT_RGB24, SWS_BICUBIC, NULL, NULL, NULL);
...
//
//
//    sprintf(szFilename, "test%d.jpg", iFrame);//Picture name for the video name + number
