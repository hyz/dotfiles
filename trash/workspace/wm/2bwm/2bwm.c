...
xcb_connection_t *conn_;             // Connection to X server.
xcb_ewmh_connection_t *ewmh_;        // Ewmh Connection.
xcb_screen_t     *screen;           // Our current screen.
int randrbase;                      // Beginning of RANDR extension events.
uint8_t curws_ = 0;                  // Current workspace.
...
void nextworkspace(){curws_==WORKSPACES-1?changeworkspace_helper(0):changeworkspace_helper(curws_+1);}
void prevworkspace(){curws_>0?changeworkspace_helper(curws_-1):changeworkspace_helper(WORKSPACES-1);}
...
void centerpointer(xcb_drawable_t win, struct client *cl)
{
    ...
    xcb_warp_pointer(conn_, XCB_NONE, win, 0, 0, 0, 0, cur_x, cur_y);
}

void updateclientlist(void)
{
    /* can only be called after the first window has been spawn */
    xcb_query_tree_reply_t *reply = xcb_query_tree_reply(conn_,xcb_query_tree(conn_, screen->root), 0);
    xcb_delete_property(conn_, screen->root, ATOM[atom_client_list]);
    xcb_delete_property(conn_, screen->root, ATOM[atom_client_list_st]);
    if (NULL == reply){
        addtoclientlist(0);
        return;
    }
...
void movepointerback(const int16_t startx, const int16_t starty, const struct client *client)
{
    if (startx>(0-conf.borderwidth-1) && startx<(client->width+conf.borderwidth+1) 
        ...
        xcb_warp_pointer(conn_, XCB_NONE, client->id,0,0,0,0,startx, starty);
}
...
{                                   // We don't need to bother mapping all windows we know about. They
    ...
    xcb_set_input_focus(conn_, XCB_NONE,XCB_INPUT_FOCUS_POINTER_ROOT,XCB_CURRENT_TIME);
    delallitems(wslist, NULL);
    xcb_ewmh_connection_wipe(ewmh_);
    xcb_flush(conn_);
    if (NULL!=ewmh_)   free(ewmh_);
...
{                                   // Get EWWM hint so we might know what workspace window win should be visible on.
    ...
    uint32_t wsp;
    xcb_get_property_cookie_t cookie = xcb_get_property(conn_, false, win, ATOM[atom_desktop],
        XCB_GET_PROPERTY_TYPE_ANY, 0, sizeof(uint32_t));
    reply = xcb_get_property_reply(conn_, cookie, NULL);
...
{                                   // check if the window is unkillable, if yes return true
    uint8_t wsp;
    xcb_get_property_cookie_t cookie = xcb_get_property(conn_, false, win, ATOM[atom_unkillable],
        XCB_GET_PROPERTY_TYPE_ANY, 0,sizeof(uint8_t));
    xcb_get_property_reply_t *reply  = xcb_get_property_reply(conn_, cookie, NULL);
...
void check_name(struct client *client)
{
    ...
    xcb_get_property_reply_t *reply = xcb_get_property_reply(conn_, xcb_get_property(conn_, false, client->id, getatom(LOOK_INTO) ,XCB_GET_PROPERTY_TYPE_ANY, 0,60), NULL);
    if (NULL==reply ||0 == xcb_get_property_value_length(reply)){
        if (NULL!=reply) free(reply);
        return;
    }
    ...
    for(unsigned int i=0;i<sizeof(ignore_names)/sizeof(__typeof__(*ignore_names));i++)
        if (strstr(wm_name_window, ignore_names[i]) !=NULL) {
            ...
            uint32_t values[1]     = {0};
            xcb_configure_window(conn_, client->id, XCB_CONFIG_WINDOW_BORDER_WIDTH, values);
            break;
        }
    free(wm_name_window);
}
...
{                                   // Add a window, specified by client, to workspace ws.
    ...
    if (!client->fixed)  /* Set window hint property so we can survive a crash. Like "fixed" */
        xcb_change_property(conn_, XCB_PROP_MODE_REPLACE, client->id,ATOM[atom_desktop], XCB_ATOM_CARDINAL, 32, 1,&ws);
}
static void addtoclientlist(const xcb_drawable_t id) 
{
    xcb_change_property(conn_, XCB_PROP_MODE_APPEND , screen->root, ATOM[atom_client_list] , XCB_ATOM_WINDOW, 32, 1,&id);
    xcb_change_property(conn_, XCB_PROP_MODE_APPEND , screen->root, ATOM[atom_client_list_st] , XCB_ATOM_WINDOW, 32, 1,&id);
}

void changeworkspace_helper(const uint32_t ws)// Change current workspace to ws
{
    xcb_change_property(conn_, XCB_PROP_MODE_REPLACE, screen->root, ATOM[atom_current_desktop], XCB_ATOM_CARDINAL, 32, 1,&ws);
    if (ws == curws_) return;
    struct client *client;
    for (struct item *item = wslist[curws_]; item != NULL; item = item->next) {
    /* Go through list of current ws. Unmap everything that isn't fixed. */
        client = item->data;
        setborders(client,false);
        if (!client->fixed) xcb_unmap_window(conn_, client->id);
    }
    for (struct item *item = wslist[ws]; item != NULL; item = item->next) {
        client = item->data;
        if (!client->fixed && !client->iconic) xcb_map_window(conn_, client->id);
    }
    curws_ = ws;
    xcb_query_pointer_reply_t *pointer = xcb_query_pointer_reply(conn_, xcb_query_pointer(conn_, screen->root), 0);
...
{                                   // Fix or unfix a window client from all workspaces. If setcolour is
    ...
    if (client->fixed) {
        client->fixed = false;
        xcb_change_property(conn_, XCB_PROP_MODE_REPLACE, client->id,ATOM[atom_desktop], XCB_ATOM_CARDINAL, 32, 1,&curws_);
        /* Delete from all workspace lists except current. */
        for (uint32_t ws = 0; ws < WORKSPACES; ws ++)
            if (ws != curws_) delfromworkspace(client, ws);
    }
    else {
        ...
        xcb_change_property(conn_, XCB_PROP_MODE_REPLACE, client->id,ATOM[atom_desktop], XCB_ATOM_CARDINAL, 32, 1,&ww);
        for (uint32_t ws = 0; ws < WORKSPACES; ws ++) /* Add window to all workspace lists. */
            if (ws != curws_) addtoworkspace(client, ws);
    }
...
{                                   // Make unkillable or killable a window client. If setcolour is
    ...
    if (client->unkillable) {
        ...
        xcb_delete_property(conn_, client->id, ATOM[atom_unkillable]);
    }
    else {
        raisewindow(client->id);
        client->unkillable = true;
        xcb_change_property(conn_, XCB_PROP_MODE_REPLACE, client->id, ATOM[atom_unkillable] , XCB_ATOM_CARDINAL, 8, 1, &client->unkillable);
    }
    setborders(client,true);
}
...
{
    if (NULL == focuswin||focuswin->fixed||arg->i == curws_) return;
    addtoworkspace(focuswin, arg->i);
    delfromworkspace(focuswin, curws_);
    xcb_unmap_window(conn_, focuswin->id);
    xcb_flush(conn_);
}
...
void sendtonextworkspace(const Arg *arg)
{
    Arg arg2 = {.i=0};
    Arg arg3 = {.i=curws_+1};
    curws_==WORKSPACES-1?sendtoworkspace(&arg2):sendtoworkspace(&arg3);
}
...
void sendtoprevworkspace(const Arg *arg)
{
    ...
    curws_>0?sendtoworkspace(&arg2):sendtoworkspace(&arg3);
}
...
{                                   // on the screen.
    ...
    addtoworkspace(client, curws_); /* Add this window to the current workspace. */
    ...
    fitonscreen(client);
    xcb_map_window(conn_, client->id);                     /* Show window on screen. */
    long data[] = { XCB_ICCCM_WM_STATE_NORMAL, XCB_NONE };/* Declare window normal. */
    xcb_change_property(conn_, XCB_PROP_MODE_REPLACE, client->id,ATOM[wm_state], ATOM[wm_state], 32, 2, data);
...
{                                   // Set border colour, width and event mask for window.
    ...
    if (xcb_ewmh_get_wm_window_type_reply(ewmh_, xcb_ewmh_get_wm_window_type(ewmh_, win), &win_type, NULL) == 1)
        for (unsigned int i = 0; i < win_type.atoms_len; i++) {
            xcb_atom_t a = win_type.atoms[i];
            if (a == ewmh_->_NET_WM_WINDOW_TYPE_TOOLBAR ||a == ewmh_->_NET_WM_WINDOW_TYPE_DOCK ) {
    ...
    xcb_change_window_attributes(conn_, win, XCB_CW_BACK_PIXEL, &conf.empty_col);
    values[0] = XCB_EVENT_MASK_ENTER_WINDOW;
    xcb_change_window_attributes_checked(conn_, win, XCB_CW_EVENT_MASK, values);
    xcb_change_save_set(conn_, XCB_SET_MODE_INSERT, win);/* Add this window to the X Save Set. */
    ...
    xcb_icccm_get_wm_normal_hints_reply(conn_,xcb_icccm_get_wm_normal_hints_unchecked(conn_, win), &hints, NULL);
...
{                                   // wrapper to get xcb keycodes from keysymbol
    ...
    if (!(keysyms = xcb_key_symbols_alloc(conn_))) return NULL;
...
{                                   // the wm should listen to key presses
    ...
    xcb_ungrab_key(conn_, XCB_GRAB_ANY, screen->root, XCB_MOD_MASK_ANY);
    for (unsigned int i=0; i<LENGTH(keys); i++) {
        keycode = xcb_get_keycodes(keys[i].keysym);
        for (unsigned int k=0; keycode[k] != XCB_NO_SYMBOL; k++)
            for (unsigned int m=0; m<LENGTH(modifiers); m++)
                xcb_grab_key(conn_, 1, screen->root, keys[i].mod | modifiers[m], keycode[k], XCB_GRAB_MODE_ASYNC, XCB_GRAB_MODE_ASYNC);
...
{
    xcb_get_modifier_mapping_reply_t *reply = xcb_get_modifier_mapping_reply(conn_, xcb_get_modifier_mapping_unchecked(conn_), NULL);
    if (!reply)
        return false;
...
{                                   // Returns true on success.
    ...
    struct client *client;
    uint32_t ws;
    xcb_query_tree_reply_t *reply = xcb_query_tree_reply(conn_,xcb_query_tree(conn_, screen->root), 0);/* Get all children. */
    ...
    for (uint32_t i = 0; i < len; i ++) {    /* Set up all windows on this root. */
        attr = xcb_get_window_attributes_reply(conn_, xcb_get_window_attributes(conn_, children[i]), NULL);
        ...
        if (!attr->override_redirect && attr->map_state == XCB_MAP_STATE_VIEWABLE) {
            ...
            if (NULL != client) {
                ...
                if (ws == NET_WM_FIXED) {
                    addtoworkspace(client, curws_); /* Add to current workspace. */
                    fixwindow(client);             /* Add to all other workspaces. */
                } else {
                    if (TWOBWM_NOWS != ws && ws < WORKSPACES) {
                        addtoworkspace(client, ws);
                        if (ws != curws_)
                            xcb_unmap_window(conn_, client->id); /* If it's not our current workspace, hide it. */
                    } else {
                        addtoworkspace(client, curws_); 
...
{                                   // events.
    const xcb_query_extension_reply_t *extension = xcb_get_extension_data(conn_, &xcb_randr_id);
...
{
    xcb_randr_get_screen_resources_current_cookie_t rcookie = xcb_randr_get_screen_resources_current(conn_, screen->root);
    xcb_randr_get_screen_resources_current_reply_t *res = xcb_randr_get_screen_resources_current_reply(conn_, rcookie, NULL);
    if (NULL == res) return;
...
{                                   // Walk through all the RANDR outputs (number of outputs == len) there
    ...
    for (int i = 0; i < len; i++) ocookie[i] = xcb_randr_get_output_info(conn_, outputs[i], timestamp);
    for (int i = 0; i < len; i ++) { /* Loop through all outputs. */
        if ((output = xcb_randr_get_output_info_reply(conn_, ocookie[i], NULL)) == NULL)
            continue;
        ...
        if (XCB_NONE != output->crtc) {
            icookie = xcb_randr_get_crtc_info(conn_, output->crtc, timestamp);
            crtc    = xcb_randr_get_crtc_info_reply(conn_, icookie, NULL);
...
void raisewindow(xcb_drawable_t win)// Raise window win to top of stack.
{
    ...
    xcb_configure_window(conn_, win,XCB_CONFIG_WINDOW_STACK_MODE,values);
    xcb_flush(conn_);
}
...
{                                   // Set window client to either top or bottom of stack depending on where it is now.
    ...
    xcb_configure_window(conn_, focuswin->id,XCB_CONFIG_WINDOW_STACK_MODE,values);
    xcb_flush(conn_);
}
...
{                                    // Move window win to root coordinates x,y.
    ...
    xcb_configure_window(conn_, win, XCB_CONFIG_WINDOW_X|XCB_CONFIG_WINDOW_Y, values);
    xcb_flush(conn_);
}
...
{                                   // Change focus to next in window ring.
    ...
    if (NULL == wslist[curws_]) return;
    /* If we currently have no focus focus first in list. */
    if (NULL == focuswin || NULL == focuswin->wsitem[curws_]){
        client  = wslist[curws_]->data;
        while (client->iconic==true) {
            client = client->wsitem[curws_]->next->data;
            if (client->wsitem[curws_]->next== NULL) break;
        }
    }
    else {
        if (arg) {
            if (NULL == focuswin->wsitem[curws_]->prev) {
                /* We were at the head of list. Focusing on last window in list unless we were already there.*/ 
                client = wslist[curws_]->data;
                /* Go to the end of the list */
                while( client->wsitem[curws_]->next!=NULL )
                    client = client->wsitem[curws_]->next->data;
                /* walk backward until we find a windows that isn't iconic */
                while(client->iconic==true)
                    client = client->wsitem[curws_]->prev->data;
            }
            else
                if(focuswin!=wslist[curws_]->data) {
                    client = focuswin->wsitem[curws_]->prev->data;
                    while (client->iconic == true && client->wsitem[curws_]->prev!=NULL)
                        client = client->wsitem[curws_]->prev->data;
            ...
            if (NULL == focuswin->wsitem[curws_]->next) {
                /* We were at the end of list. Focusing on first window in list unless we were already there. */
                client = wslist[curws_]->data;
                while(client->iconic && client->wsitem[curws_]->next!=NULL)
                    client = client->wsitem[curws_]->next->data;
            }
            else {
                client = focuswin->wsitem[curws_]->next->data;
                while (client->iconic==true && client->wsitem[curws_]->next!=NULL)
                    client = client->wsitem[curws_]->next->data;
...
//    xcb_set_input_focus(conn_, XCB_NONE, XCB_INPUT_FOCUS_NONE,XCB_CURRENT_TIME);
...
void setfocus(struct client *client)// Set focus on window client.
{
    ...
    if (NULL == client) {
        ...
        xcb_change_property(conn_, XCB_PROP_MODE_REPLACE, screen->root, ATOM[atom_focus] , XCB_ATOM_WINDOW, 32, 1,&not_win);
        xcb_flush(conn_);
        return;
    }   
    ...
    xcb_change_property(conn_, XCB_PROP_MODE_REPLACE, client->id,ATOM[wm_state], ATOM[wm_state], 32, 2, data);
    xcb_set_input_focus(conn_, XCB_INPUT_FOCUS_POINTER_ROOT, client->id,XCB_CURRENT_TIME); /* Set new input focus. */
    xcb_change_property(conn_, XCB_PROP_MODE_REPLACE, screen->root, ATOM[atom_focus] , XCB_ATOM_WINDOW, 32, 1,&client->id);
...
void moveresize(xcb_drawable_t win, const uint16_t x, const uint16_t y,const uint16_t width, const uint16_t height)
{
    ...
    xcb_configure_window(conn_, win, XCB_CONFIG_WINDOW_X | XCB_CONFIG_WINDOW_Y
...
{                                   // Resize window win to width,height.
    ...
    xcb_configure_window(conn_, win,XCB_CONFIG_WINDOW_WIDTH|XCB_CONFIG_WINDOW_HEIGHT, values);
    xcb_flush(conn_);
}
...
{                                  // Try to snap to other windows and monitor border
    ...
    for (item = wslist[curws_]; item != NULL; item = item->next) {
        win = item->data;
        if (client != win) {
            ...
            if (abs((client->x + client->width) - win->x+conf.borderwidth) < borders[2])
                if (client->y + client->height > win->y && client->y < win->y + win->height)
                    client->x = (win->x - client->width) - (2 * conf.borderwidth);

            if (abs((client->y + client->height) - win->y+conf.borderwidth) < borders[2])
                if (client->x + client->width >win->x && client->x < win->x + win->width)
...
{                                   // Move window win as a result of pointer motion to coordinates rel_x,rel_y.
    if(focuswin==NULL||NULL == focuswin->wsitem[curws_])
        return;
    focuswin->x = rel_x;
    focuswin->y = rel_y;
    if (borders[2] >0 )
...
void setborders(struct client *client,const bool isitfocused)
{
    ...
    xcb_pixmap_t pmap = xcb_generate_id(conn_);
    /* my test have shown that drawing the pixmap directly on the root window is faster then drawing it on the window directly */
    xcb_create_pixmap(conn_, screen->root_depth, pmap, screen->root, client->width+(conf.borderwidth*2), client->height+(conf.borderwidth*2));
    xcb_gcontext_t gc = xcb_generate_id(conn_);
    xcb_create_gc(conn_, gc, pmap, 0, NULL);
    if (inv) {
    ...
    xcb_change_gc(conn_, gc, XCB_GC_FOREGROUND, &values[0]);
    xcb_poly_fill_rectangle(conn_, pmap, gc, 5, rect_outer);
    
    values[0]   = conf.focuscol;
    if (!isitfocused)  values[0]  = conf.unfocuscol;
    xcb_change_gc(conn_, gc, XCB_GC_FOREGROUND, &values[0]);
    xcb_poly_fill_rectangle(conn_, pmap, gc, 5, rect_inner);
    values[0] = pmap;
    xcb_change_window_attributes(conn_,client->id, XCB_CW_BORDER_PIXMAP, &values[0]);
    /* free the memory we allocated for the pixmap */
    xcb_free_pixmap(conn_,pmap);
...
void maximize(const Arg *arg) { fullscreen(arg); }
...
void maxvert_hor(const Arg *arg)
{
    ...
    if (arg->i==1) {
        ...
        xcb_configure_window(conn_, focuswin->id, XCB_CONFIG_WINDOW_Y
        ...
        xcb_configure_window(conn_, focuswin->id, XCB_CONFIG_WINDOW_X
...
void hide()
{
    ...
    xcb_unmap_window(conn_, focuswin->id);
    xcb_change_property(conn_, XCB_PROP_MODE_REPLACE, focuswin->id, ATOM[wm_state], ATOM[wm_state], 32, 3, data); 
    xcb_flush(conn_);
}

bool getpointer(const xcb_drawable_t *win, int16_t *x, int16_t *y)
{
    xcb_query_pointer_reply_t *pointer = xcb_query_pointer_reply(conn_, xcb_query_pointer(conn_, *win), 0);
...
{
    xcb_get_geometry_reply_t *geom = xcb_get_geometry_reply(conn_, xcb_get_geometry(conn_, *win), NULL);
...
{
    if (NULL == focuswin|| NULL == wslist[curws_]|| focuswin->maxed) return;
...
void deletewin()
{
    ...
    xcb_get_property_cookie_t cookie = xcb_icccm_get_wm_protocols_unchecked(conn_, focuswin->id,ATOM[wm_protocols]);
    if (focuswin->id == top_win)
        top_win = 0;
    if (xcb_icccm_get_wm_protocols_reply(conn_, cookie, &protocols, NULL) == 1){
        for (uint32_t i = 0; i < protocols.atoms_len; i++)
            if (protocols.atoms[i] == ATOM[wm_delete_window]) {
                ...
                xcb_send_event(conn_, false, focuswin->id,XCB_EVENT_MASK_NO_EVENT, (char *) &ev);
                use_delete = true;
                break;
            }
        xcb_icccm_get_wm_protocols_reply_wipe(&protocols);
    }
    if (!use_delete) xcb_kill_client(conn_, focuswin->id);
}
...
{                                   // Function to make the cursor move with the keyboard
    ...
    if      (cases==0)
        xcb_warp_pointer(conn_, XCB_NONE, XCB_NONE, 0, 0, 0, 0, 0, -speed);
    else if (cases==1)
        xcb_warp_pointer(conn_, XCB_NONE, XCB_NONE, 0, 0, 0, 0, 0, speed);
    else if (cases==2)
        xcb_warp_pointer(conn_, XCB_NONE, XCB_NONE, 0, 0, 0, 0, speed, 0);
    else if (cases==3)
        xcb_warp_pointer(conn_, XCB_NONE, XCB_NONE, 0, 0, 0, 0, -speed, 0);
    xcb_flush(conn_);
}
...
{                                    // wrapper to get xcb keysymbol from keycode
    xcb_key_symbols_t *keysyms;
    if (!(keysyms = xcb_key_symbols_alloc(conn_))) return 0;
...
void circulaterequest(xcb_generic_event_t *ev)
{
    ...
    xcb_circulate_window(conn_, e->window, e->place);
}
...
void configurerequest(xcb_generic_event_t *ev)
{
    ...
    if ((client = findclient(&e->window))) { /* Find the client. */
        ...
        if (e->value_mask & XCB_CONFIG_WINDOW_SIBLING) {
            values[0] = e->sibling;
            xcb_configure_window(conn_, e->window,XCB_CONFIG_WINDOW_SIBLING,values);
        }
        if (e->value_mask & XCB_CONFIG_WINDOW_STACK_MODE) {
            values[0] = e->stack_mode;
            xcb_configure_window(conn_, e->window,XCB_CONFIG_WINDOW_STACK_MODE,values);
        }
...
struct client create_back_win(void)
{
    ...
    temp_win.id = xcb_generate_id(conn_);
    uint32_t values[1] = {conf.focuscol};
    xcb_create_window (conn_, XCB_COPY_FROM_PARENT,/* depth */ temp_win.id, /* window Id */  screen->root, /* parent window */
...
#ifndef COMPTON
    values[0]=1;
    xcb_change_window_attributes(conn_, temp_win.id, XCB_BACK_PIXMAP_PARENT_RELATIVE, values);
#else
    values[0] = conf.unfocuscol;
    xcb_change_window_attributes(conn_, temp_win.id, XCB_CW_BACK_PIXEL, values);
#endif
...
static void mousemotion(const Arg *arg)
{
    ...
    if(arg->i == TWOBWM_MOVE)
        cursor = Create_Font_Cursor (conn_, 52 ); /* fleur */
    else {                   
        cursor  = Create_Font_Cursor (conn_, 120); /* sizing */
        example = create_back_win();
        xcb_map_window(conn_,example.id);
    }
    xcb_grab_pointer_reply_t *grab_reply = xcb_grab_pointer_reply(conn_
            , xcb_grab_pointer(conn_, 0, screen->root, BUTTONMASK|XCB_EVENT_MASK_BUTTON_MOTION|XCB_EVENT_MASK_POINTER_MOTION, XCB_GRAB_MODE_ASYNC, XCB_GRAB_MODE_ASYNC, XCB_NONE, cursor, XCB_CURRENT_TIME)
            , NULL);
    if (grab_reply->status != XCB_GRAB_STATUS_SUCCESS) {
        free(grab_reply);
        if (arg->i == TWOBWM_RESIZE)
            xcb_unmap_window(conn_,example.id);
        return;
    }
    free(grab_reply);
    ...
    do {
        if (NULL!=e)
            free(e);
        while(!(e = xcb_wait_for_event(conn_)))
            xcb_flush(conn_);
...
void unmapnotify(xcb_generic_event_t *ev)
{
    ...
    * If we do this, we need to keep track of our own windows and ignore UnmapNotify on them.*/
    ...
    if (NULL == client || client->wsitem[curws_]==NULL) return;
...
void run(void)
{
    ...
    while (0 == sigcode) { /* the WM is running */
        ...
        if (xcb_connection_has_error(conn_))
            abort();
        if ((ev = xcb_wait_for_event(conn_))) {
...
{
    xcb_intern_atom_cookie_t atom_cookie = xcb_intern_atom(conn_, 0, strlen(atom_name), atom_name);
    xcb_intern_atom_reply_t *rep = xcb_intern_atom_reply(conn_, atom_cookie, NULL);
...
void grabbuttons(struct client *c)  // set the given client to listen to button events (presses / releases)
{
    ...
    for (unsigned int b=0; b<LENGTH(buttons); b++)
        for (unsigned int m=0; m<LENGTH(modifiers); m++)
            xcb_grab_button(conn_, 1, c->id, XCB_EVENT_MASK_BUTTON_PRESS, XCB_GRAB_MODE_ASYNC, XCB_GRAB_MODE_ASYNC,
                screen->root, XCB_NONE, buttons[b].button, buttons[b].mask|modifiers[m]);
}

void ewmh_init(void)
{
    if (!(ewmh_ = calloc(1, sizeof(xcb_ewmh_connection_t))))    printf("Fail\n");
    xcb_intern_atom_cookie_t *cookie = xcb_ewmh_init_atoms(conn_, ewmh_);
    xcb_ewmh_init_atoms_replies(ewmh_, cookie, (void *)0);
}

bool setup()
{
    int scrno;
    if (!xcb_connection_has_error(conn_ = xcb_connect(NULL, &scrno)))
        return false;
    screen = xcb_screen_of_display(conn_, scrno);
    if (!screen)
        return false;
    xcb_window_t cwin = xcb_generate_id(conn_);
    uint32_t event_mask_pointer[] = {XCB_EVENT_MASK_POINTER_MOTION};
    xcb_create_window(conn_, XCB_COPY_FROM_PARENT, cwin, screen->root, 0, 0, screen->width_in_pixels, screen->height_in_pixels
            , 0, XCB_WINDOW_CLASS_INPUT_OUTPUT, XCB_COPY_FROM_PARENT,XCB_CW_EVENT_MASK, event_mask_pointer);
    ewmh_init();
    xcb_ewmh_set_wm_pid(ewmh_, cwin, getpid());
    xcb_ewmh_set_wm_name(ewmh_, cwin, 4, "2bwm");
    xcb_ewmh_set_supporting_wm_check(ewmh_, cwin, cwin);
    xcb_ewmh_set_supporting_wm_check(ewmh_, screen->root, cwin);
    xcb_atom_t net_atoms[] = {
        ewmh_->_NET_SUPPORTED,             ewmh_->_NET_WM_DESKTOP,
        ewmh_->_NET_NUMBER_OF_DESKTOPS,    ewmh_->_NET_CURRENT_DESKTOP,
        ewmh_->_NET_ACTIVE_WINDOW,         ewmh_->_NET_WM_ICON,
        ewmh_->_NET_WM_STATE,              ewmh_->_NET_WM_NAME,
        ewmh_->_NET_SUPPORTING_WM_CHECK,   ewmh_->_NET_WM_STATE_HIDDEN,
        ewmh_->_NET_WM_ICON_NAME,          ewmh_->_NET_WM_WINDOW_TYPE,
        ewmh_->_NET_WM_WINDOW_TYPE_DOCK,   ewmh_->_NET_WM_WINDOW_TYPE_TOOLBAR,
        ewmh_->_NET_WM_PID
    };
    xcb_ewmh_set_supported(ewmh_, scrno, LENGTH(net_atoms), net_atoms);
    ...
    xcb_generic_error_t *error = xcb_request_check(conn_, xcb_change_window_attributes_checked(conn_, screen->root, XCB_CW_EVENT_MASK, values));
    xcb_flush(conn_);
    if (error)
        return false;
    xcb_change_property(conn_, XCB_PROP_MODE_REPLACE, screen->root, ATOM[atom_current_desktop], XCB_ATOM_CARDINAL, 32, 1,&curws_);
    xcb_change_property(conn_, XCB_PROP_MODE_REPLACE, screen->root, ATOM[atom_nb_workspace]   , XCB_ATOM_CARDINAL, 32, 1,&_WORKSPACES);
    grabkeys();
    /* set events */
